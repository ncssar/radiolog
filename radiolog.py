		...
		with self.radioMarkerDictLock:
			if d:
				...
				if lat: # latitude specified: it's a new call: update lat, lon, latestTimeString, label; set lastId to None
					...
					d['label']=label
					logging.info(f'  location update: {label} {latestTimeString} lon={lon} lat={lat}')
				...
				logging.info(f'  new entry added: {label} {latestTimeString} lon={lon} lat={lat}')
	...
	def _radioMarkerWorker(self,event):
		...
		while True:
			...
			try:
				...
				logging.error(f'_radioMarkerWorker: outer exception caught in order to keep the thread alive: {e}')
			# finally: # cleanup even if there was an early exit
			# 	pass
...
##		self.throb(0)
	...
	def customFocusInEvent(self,widget):
		...
		if widget.objectName()=='teamField':
			self.teamFieldTextChanged() # this shows the callsign change slider if needed
			self.parent.pendingActivationChange=False # clear it here always, since it was just checked
...
# 		logging.info("  quickTextAction called: text="+str(quickText))
	...
	def promptForCallsign(self):
		...
		self.callsignGroupBoxesShowHide(show='firstCall',source='promptForCallsign')
...
##		self.setWindowTitle("Radio Log - "+tmpTxt+" - "+self.ui.to_fromField.currentText()+" "+self.ui.teamField.text())
	...
	def callsignGroupBoxesShowHide(self,show='none',animate=True,source=''):
		suffix=f'  from {source}' if source else ''
		logging.info(f'showHide called: show={show}  animate={animate}{suffix}')
