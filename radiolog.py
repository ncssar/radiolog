...
##				widget.timer.start(newEntryDialogTimeoutSeconds*1000) # reset the timeout
	...
	def sendRadioMarker(self,fleet,dev,uid,callsign,lat,lon):
		...
		#    values are corresponding caltopo feature IDs
		
		if uid:
			deviceStr=str(uid)
		else:
			deviceStr=str(fleet)+':'+str(dev)
		existingId=self.radioMarkerDict.get(deviceStr,None)
		...
		if self.cts:
			...
			id=self.cts.addMarker(lat,lon,self.getRadioMarkerLabelForCallsign(callsign),latestTimeString,folderId=self.radioMarkerFID,existingId=existingId)
		self.radioMarkerDict[deviceStr]=id # queued markers will be added during createCTS
...
class changeCallsignDialog(QDialog,Ui_changeCallsignDialog):
	...
	def accept(self):
		...
		existingMarkerID=self.parent.parent.radioMarkerDict.get(deviceStr,None)
		if existingMarkerID:
